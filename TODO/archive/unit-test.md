# 单元测试待办清单

## 已完成测试

| 文件                    | 函数              | 覆盖率 |
| ----------------------- | ----------------- | ------ |
| `help/format.ts`        | `iconMap()`        | 100%   |
| `help/index.ts`         | `typedBoolean()`   | 100%   |
| `help/linear.ts`        | `buildCommentBody()` | 100% |
| `help/linear.ts`        | `sendPreview()`    | 100%   |
| `help/linear.ts`        | `releaseIssues()`   | 100%   |
| `help/logger.ts`        | `createLogger()`   | 100%   |
| `help/redis.ts`         | `upstashCache()`   | 100%   |
| `help/mihomo.ts`        | `findProxyChain()` | 100%   |
| `help/mihomo.ts`        | `findCurrentProxy()` | 100% |
| `help/mihomo.ts`        | `pickProxy()`      | 100%   |
| `help/mihomo.ts`        | `getDelay()`       | 100%   |
| `help/mihomo-prompts.ts`  | `getProxyDelay()`  | 100%   |
| `help/mihomo-prompts.ts`  | `delayLevel()`     | 100%   |
| `help/mihomo-prompts.ts`  | `choices()`        | 100%   |
| `help/mihomo-prompts.ts`  | `getChildren()`    | 100%   |
| `help/mihomo-prompts.ts`  | `searchProxy()`    | 100%   |
| `help/io.ts`            | `expandAlias()`    | 100%   |
| `help/io.ts`            | `showAvailableActions()` | 100% |
| `help/cli.ts`            | `cli()`            | 100%   |
| `help/cli.ts`            | `gitShowRef()`     | 100%   |
| `help/util.ts`           | `pbcopy()`         | 100%   |
| `help/io.ts`             | `showSubcommands()` | 100% |
| `help/io.ts`             | `_require()`       | 100%   |
| `help/io.ts`             | `showHelp()`       | 100%   |
| `help/io.ts`             | `main()`           | 100%   |
| `help/git.ts`            | `findNextBranch()` | 100%   |

---

## 待测试 - 纯函数（优先级高）

这些函数是纯函数，不依赖外部系统，可以直接测试。

（暂无）

---

## 待测试 - 需要 Mock（优先级中）

这些函数依赖外部系统，需要使用 `vitest` 的 mock 功能。

（暂无 - 所有需要 mock 的函数已完成测试）

### 已完成 ✅

- ✅ `help/logger.ts` - `createLogger()` - 已测试（`test/help/logger.test.ts`）
- ✅ `help/redis.ts` - `upstashCache()` - 已测试（`test/help/redis.test.ts`）
  - ✅ `get(key, cacheTime, force)`
  - ✅ `remove(key)`
- ✅ `help/linear.ts` - `sendPreview()` - 已测试（`test/help/linear.test.ts`）
- ✅ `help/linear.ts` - `releaseIssues()` - 已测试（`test/help/linear.test.ts`）

---

## Mock 示例

```typescript
import { describe, expect, test, vi } from "vitest";

// Mock 模块（必须在导入之前调用）
vi.mock("@/fetch/mihomo", () => ({
  mihomo: vi.fn(),
}));

// 导入 mock 模块
import { mihomo } from "@/fetch/mihomo";

// 在测试中使用 vi.mocked() 进行类型安全的访问
const mihomoMock = vi.mocked(mihomo);
mihomoMock.mockResolvedValue({ proxies: {} });
```

---

## 运行测试

```bash
bun run test                    # 运行所有测试（带覆盖率）
bun run test:watch              # 监听模式
bun run test:ui                 # UI 模式
```
