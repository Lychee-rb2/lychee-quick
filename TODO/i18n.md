...
### Phase 6: 脚本消息（低优先级，4 个文件）✅ 已完成

- ✅ `scripts/utils/installCli.ts` — 导入 `t`，替换 4 处 console 消息：
  - `script.installCli.binUpdated` — bin 更新提示
  - `script.installCli.writeFailed` — 写入失败
  - `script.installCli.installed` — 安装成功
  - `script.installCli.installFailed` — 安装失败
- ✅ `scripts/utils/installZshCompletion.ts` — 导入 `t`，替换 5 处 console 消息：
  - `script.zshCompletion.alreadyConfigured` — 已配置
  - `script.zshCompletion.installed` — 安装成功
  - `script.zshCompletion.installFailed` — 安装失败
  - `script.zshCompletion.zshNotDetected` — 未检测到 Zsh
  - `script.zshCompletion.enableHint` — 启用提示
- ✅ `scripts/utils/buildGlobalEnv.ts` — 导入 `t`，替换 1 处消息：
  - `script.buildGlobalEnv.generated` — 生成完成提示
- ✅ `scripts/utils/getCompletions.ts` — 导入 `t`，替换 1 处错误消息：
  - `script.getCompletions.dashInName` — 命令名包含 "-" 错误
- ✅ `i18n/zh.json` 新增 `script.*` (11 个键)
- ✅ `i18n/en.json` 新增 `script.*` (11 个键)
- ✅ `test/scripts/getCompletions.test.ts` 同步更新：mock `@/i18n`，更新错误断言
- ✅ `test/scripts/installCli.test.ts` 同步更新：mock `@/i18n`
- ✅ `test/scripts/buildGlobalEnv.test.ts` 同步更新：mock `@/i18n`
- ✅ `test/scripts/installZshCompletion.test.ts` 同步更新：mock `@/i18n`
- ✅ 全部 439 个测试通过，0 个 lint 错误

### Phase 7: 其他 ✅ 已完成

- ✅ `help/linear.ts` — 导入 `t`，替换 release note 标题为 `t("app.linear.release.noteTitle", { today })`
- ✅ `i18n/zh.json` 新增 `app.linear.release.noteTitle`
- ✅ `i18n/en.json` 新增 `app.linear.release.noteTitle`
- ✅ `test/help/linear.test.ts` 同步更新：mock `@/i18n`，更新断言
- ✅ 全部 439 个测试通过，0 个 lint 错误
